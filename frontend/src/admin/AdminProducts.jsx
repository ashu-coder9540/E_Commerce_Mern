          ...
          {products.map((items, index) => (
            <div key={index} className="bg-white rounded-xl shadow p-4 hover:shadow-xl transition">
              <img
                ...
                alt="Product Image"
                className="w-full h-40 object-contain rounded-lg mb-4 border"
              />
              ...
              <div className="flex flex-col sm:flex-row justify-between mt-4">
                <Link to={`/admin/edit-product/${items._id}`} className="flex items-center gap-2 text-blue-500 hover:text-blue-700">
                  <FaEdit /> Edit
                </Link>
                <Link onClick={()=>{handleDelete(items._id)}} className="flex items-center gap-2 text-red-500 hover:text-red-700">
                  <FaTrash /> Delete
                </Link>
              </div>
            </div>
